...
CREATE FUNCTION getArtistId() RETURNS INTEGER DETERMINISTIC return 1//

DELIMITER ;

#returns the genre of the artist
Drop view if exists Artist_Genre;
CREATE VIEW Artist_Genre AS
select A.genre as genre
from Artist as A
where A.artist_id = getArtistId();

#returns songs of artists from genre
Drop view if exists Songs_Genre;
